...
// A file with nothing at column 0 was read as a clean, empty tree, which is
// the one answer a scanner must never give to a file it did not understand:
// `--fail-on` goes green over a lockfile nobody read. Every sibling reader
// refuses a file that is not what its name claims, and the header yarn writes
// is the only evidence left once there are no entries to go on.
#[test]
fn an_empty_file_is_not_an_empty_lockfile() {
    for src in ["", "\n\n\n", "# just a comment\n"] {
        let err = read(src).unwrap_err();
        assert!(
            err.to_string().contains("not the lockfile"),
            "{src:?}: {err}"
        );
    }
}

// The other half: a project with no dependencies really does produce a
// header and nothing else, and that is a tree with zero packages rather than
// an error.
#[test]
fn a_header_with_no_entries_is_an_empty_tree() {
    let tree = read(
        "# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.\n# yarn lockfile v1\n\n",
    )
    .unwrap();
    assert_eq!(tree.packages.len(), 0);
}

// The header is not required of a file that has entries. Lockfiles get
// hand-edited and concatenated, and the entries are the evidence once there
// are any.
#[test]
fn entries_without_the_header_still_read() {
    let tree = read("a@^1.0.0:\n  version \"1.0.0\"\n").unwrap();
