...
of these pairs, and any threshold loose enough to catch a typo is loose enough
to catch a legitimate sibling. Precision collapses and the tool becomes noise.
...
Clause 1 is a binary search over a sorted list compiled into the binary. Clause
2 is Damerau-Levenshtein, the unrestricted Lowrance-Wagner variant rather than
the optimal-string-alignment version most libraries ship under that name, with
the threshold at 2. Clause 3 is a lookup in a vector of in-degrees built from
the lockfile's own edges.
...
`npm-xl` fixture that ordering is most of the difference between 380 ms and
...
audit; "the file an LLM helped write lists this package" is not evidence that
the package exists. Those go into a separate `roots` list, not into `edges`.
...
dependencies for a 582-package project. A hallucinated name added to
...
real registry entries, and precision on the fixtures falls off a cliff. Two
still catches every single-character slip — deletion, insertion, substitution,
...
population of legitimate siblings, and the threshold that catches it catches
half the registry with it.
